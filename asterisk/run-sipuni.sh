...
# OneLink/Fonoster isolated Sipuni overlay. Applied after Fonoster template substitution.
if [ -f /etc/asterisk/pjsip_sipuni.conf ]; then
  if ! grep -q '^#include /etc/asterisk/pjsip_sipuni.conf' /etc/asterisk/pjsip.conf; then
    {
      echo
      echo '#include /etc/asterisk/pjsip_sipuni.conf'
